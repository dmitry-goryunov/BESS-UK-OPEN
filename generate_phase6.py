...
with open(PROCESSED / "lsmc_valuation_summary.json") as f:
    lsmc_sum = json.load(f)

with open(PROCESSED / "mtm_summary.json") as f:
    mtm_sum = json.load(f)

V_LSMC    = float(lsmc_sum["mtm_gbp"]["mean"])
V_RI      = float(lsmc_sum["ri_mean_gbp"])
MTM_STD   = float(lsmc_sum["mtm_gbp"]["std"])
...
ax.set_xlabel("GBP")
ax.set_title("LSMC Optimality Bounds", fontweight="bold")
...
print(f"{'='*55}")
print(f"  V_LSMC : £{V_LSMC:>14,.0f}")
print(f"  V_dual : £{V_DUAL:>14,.0f}  (gap {GAP_PCT:.1%}  {'PASS' if DUAL_OK else 'REFINE'})")
print(f"  V_RI   : £{V_RI:>14,.0f}")
