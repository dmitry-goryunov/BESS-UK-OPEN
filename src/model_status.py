...
def build_model_status(processed_dir: str | Path) -> list[dict[str, str]]:
    ...
    mtm = lsmc.get("mtm_gbp", {}) if isinstance(lsmc.get("mtm_gbp"), dict) else {}
    ...
    if not lsmc:
        ...
        if isinstance(lsmc_mean, (int, float)):
            evidence.append(f"V_LSMC mean=GBP {lsmc_mean:,.0f}")
        if isinstance(ri_mean, (int, float)):
            evidence.append(f"V_RI mean=GBP {ri_mean:,.0f}")
